...
function getUpdatesChannel(guild, repoConfig) {
  return guild.channels.cache.find(
    (c) =>
      c.parentId === repoConfig.categoryId &&
      c.name === "git-updates" &&
...
async function handleGithubEvent(eventType, payload, client) {
  ...
  for (const repoConfig of repoGuilds) {
    ...
    try {
      ...
      console.error(
        `Error handling ${eventType} for ${repoFullName} in guild ${repoConfig.guildId}:`,
        err,
      );
    }
  }
...
async function handlePullRequest(guild, repoConfig, payload) {
  ...
  if (pr.body)
    descParts.push(`\u{1F4AC} **Description:** ${pr.body.slice(0, 200)}`);
...
async function handleIssue(guild, repoConfig, payload, client) {
  ...
  const color =
    action === "opened" ? 0x2ea44f : action === "closed" ? 0xda3633 : 0xf0883e;
  ...
  if (issue.body)
    descParts.push(`\u{1F4AC} **Description:** ${issue.body.slice(0, 200)}`);
  ...
  if (action === "opened" && client) {
    ...
    if (repoFullName) {
      try {
        ...
        console.error(
          `Failed to create bug from GitHub issue in guild ${guild.id}:`,
          err,
        );
      }
    }
...
async function autoSyncMirroredFiles(guild, repoFullName, payload) {
  ...
  const users =
    authDoc.users instanceof Map
      ? Object.fromEntries(authDoc.users)
      : authDoc.users || {};
