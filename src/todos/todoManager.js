...
async function ensureTodosCategory(guild) {
  const existing = guild.channels.cache.find(
    (c) =>
      c.type === ChannelType.GuildCategory && c.name.toLowerCase() === "todos",
  );
  if (existing) {
...
function buildTodoDetailEmbed(todo) {
  ...
  if (todo.repoName)
    embed.addFields({ name: "Repository", value: todo.repoName, inline: true });
  if (todo.assignee)
    embed.addFields({
      name: "Assignee",
      value: `<@${todo.assignee}>`,
  ...
  if (todo.dueDate)
    embed.addFields({
      name: "Due Date",
      value: `<t:${Math.floor(new Date(todo.dueDate).getTime() / 1000)}:D>`,
      inline: true,
    });
  if (todo.tags && todo.tags.length > 0)
    embed.addFields({ name: "Tags", value: todo.tags.join(", ") });
...
function scheduleDeletion(client, todoId, delayMs) {
  ...
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      console.log(
        `Deletion timer fired for todo ${todoId}, status: ${todo?.status}, channelId: ${todo?.channelId}`,
      );
      ...
      if (guild && todo.channelId) {
        ...
        if (channel) {
          ...
          console.log(
            `Channel ${todo.channelId} not found in cache for todo ${todoId}`,
          );
        }
      }
...
async function updateTodoListMessage(client, todo) {
  ...
  if (todo.todoListMessageId) {
    try {
      ...
      console.error(
        "Failed to edit todo-list message, sending new one:",
        err.message,
      );
    }
  }
...
async function backfillTodoList(client, guildId) {
  ...
  const allTodos = (await todoModel.getTodosByGuild(guildId)).filter(
    (todo) => !todo.repoName || activeRepoNames.has(todo.repoName),
  );

  // 1. Create channels for unfinished todos that don't have one
  for (const todo of allTodos) {
    if (todo.status === "done") continue;
    const existingChannel = todo.channelId
      ? guild.channels.cache.get(todo.channelId)
      : null;
...
module.exports = {
  ensureTodosCategory,
  ensureTodosCategoryForRepo,
  ensureTodosCategoryGeneral,
  ensureAddTodoChannel,
  ensureTodoListChannel,
  createTodoChannel,
  buildAddTodoEmbed,
  buildAddTodoButton,
  buildAddTodoModal,
