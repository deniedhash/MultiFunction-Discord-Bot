...
async function ensureFeaturesCategoryGeneral(guild) {
    const existing = guild.channels.cache.find(
        c => c.type === ChannelType.GuildCategory && c.name.toLowerCase() === 'features',
    );
    if (existing) {
...
async function createFeatureChannel(guild, category, feature) {
    let name = `feature-${feature.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 80)}`;
    const existing = guild.channels.cache.find(
...
function buildRepoSelectMenu(repos) {
    ...
    options.unshift({ label: 'General (Not tied to a repo)', value: 'general', description: 'A feature not associated with any specific repository.' });
...
function buildFeatureDetailEmbed(feature) {
    ...
    if (feature.repositoryName) embed.addFields({ name: 'Repository', value: feature.repositoryName, inline: true });
    if (feature.assignee) embed.addFields({ name: 'Assignee', value: `<@${feature.assignee}>`, inline: true });
    if (feature.dueDate) embed.addFields({ name: 'Due Date', value: new Date(feature.dueDate).toDateString(), inline: true });
    if (feature.tags && feature.tags.length > 0) embed.addFields({ name: 'Tags', value: feature.tags.join(', '), inline: true });
...
function buildFeatureListEmbed(feature) {
    ...
    if (feature.channelId) {
        embed.addFields({ name: 'Channel', value: `<#${feature.channelId}>`, inline: true });
    }
    return embed;
}
...
function scheduleDeletion(client, featureId, delayMs) {
    if (!delayMs) delayMs = featureDeleteDelay * 1000;
    ...
    const timer = setTimeout(async () => {
        ...
        try {
            ...
            console.log(`Deletion timer fired for feature ${featureId}, status: ${feature?.status}, channelId: ${feature?.channelId}`);
            if (!feature || (feature.status !== 'completed' && feature.status !== 'rejected')) return;
            ...
            if (guild && feature.channelId) {
                ...
                if (channel) {
                    ...
                    console.log(`Channel ${feature.channelId} not found in cache for feature ${featureId}`);
                }
            }
...
async function replayUpdatesToChannel(channel, feature) {
    ...
    await featureModel.setFeatureEmbedMessageId(feature._id.toString(), embedMsg.id);
...
async function updateFeatureListMessage(client, feature) {
    ...
    if (feature.featureListMessageId) {
        try {
            const msg = await featureListChannel.messages.fetch(feature.featureListMessageId);
            await msg.edit({ embeds: [embed], components });
            return;
        } catch (err) {
            console.error('Failed to edit feature-list message, sending new one:', err.message);
        }
    }
...
async function backfillFeatureList(client, guildId) {
    ...
    const allFeatures = (await featureModel.getFeaturesByGuild(guildId))
        .filter(feature => !feature.repositoryName || activeRepoNames.has(feature.repositoryName));

    // 1. Create channels for unresolved features that don't have one
    for (const feature of allFeatures) {
        if (feature.status === 'completed' || feature.status === 'rejected') continue;
        const existingChannel = feature.channelId ? guild.channels.cache.get(feature.channelId) : null;
    ...
    const sortedFeatures = [...allFeatures].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
...
async function createFeatureFromExternal(client, data) {
    const { guildId, repositoryId, repositoryName, title, description, priority, createdBy, dueDate, tags } = data;
...
module.exports = {
    ensureFeaturesCategoryGeneral, ensureFeaturesCategoryForRepo,
    ensureAddFeatureChannel, ensureFeatureListChannel, createFeatureChannel,
    buildAddFeatureEmbed, buildAddFeatureButton, buildAddFeatureModal, buildRepoSelectMenu,
    buildFeatureDetailEmbed, buildFeatureChannelButtons,
    buildFeatureListEmbed, buildFeatureListComponents, buildUpdateModal,
    scheduleDeletion, cancelDeletion,
    replayUpdatesToChannel, updateFeatureListMessage, backfillFeatureList, createFeatureFromExternal,
    deletionTimers,
};
