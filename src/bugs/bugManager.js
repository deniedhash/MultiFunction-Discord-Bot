...
async function ensureBugsCategory(guild) {
  const existing = guild.channels.cache.find(
    (c) =>
      c.type === ChannelType.GuildCategory && c.name.toLowerCase() === "bugs",
  );
  if (existing) {
...
function buildBugDetailEmbed(bug) {
  ...
  if (bug.repoName)
    embed.addFields({ name: "Repository", value: bug.repoName, inline: true });
  if (bug.steps)
    embed.addFields({ name: "Steps to Reproduce", value: bug.steps });
  if (bug.wipUserId)
    embed.addFields({
      name: "WIP By",
      value: `<@${bug.wipUserId}>`,
...
function scheduleDeletion(client, bugId, delayMs) {
  ...
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      console.log(
        `Deletion timer fired for bug ${bugId}, status: ${bug?.status}, channelId: ${bug?.channelId}`,
      );
      ...
      if (guild && bug.channelId) {
        ...
        if (channel) {
          ...
          console.log(
            `Channel ${bug.channelId} not found in cache for bug ${bugId}`,
          );
        }
      }
...
async function updateBugListMessage(client, bug) {
  ...
  if (bug.bugListMessageId) {
    try {
      ...
      console.error(
        "Failed to edit bug-list message, sending new one:",
        err.message,
      );
    }
  }
...
async function backfillBugList(client, guildId) {
  ...
  const allBugs = (await bugModel.getBugsByGuild(guildId)).filter((bug) =>
    activeRepoNames.has(bug.repoName),
  );

  // 1. Create channels for unresolved bugs that don't have one
  for (const bug of allBugs) {
    if (bug.status === "resolved") continue;
    const existingChannel = bug.channelId
      ? guild.channels.cache.get(bug.channelId)
      : null;
...
module.exports = {
  ensureBugsCategory,
  ensureBugsCategoryForRepo,
  ensureAddBugChannel,
  ensureBugListChannel,
  createBugChannel,
  buildAddBugEmbed,
  buildAddBugButton,
  buildAddBugModal,
  buildRepoSelectMenu,
  buildBugDetailEmbed,
  buildBugChannelButtons,
  buildBugListEmbed,
  buildBugListComponents,
  buildUpdateModal,
  scheduleDeletion,
  cancelDeletion,
  replayUpdatesToChannel,
  updateBugListMessage,
  backfillBugList,
  createBugFromExternal,
  deletionTimers,
};
